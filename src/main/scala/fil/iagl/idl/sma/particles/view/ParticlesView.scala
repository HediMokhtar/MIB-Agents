...
import fil.iagl.idl.sma.particles.model.SMA
...
class ParticlesView extends Application {
  ...
  override def start(primaryStage: Stage): Unit = {
    ...
    val model = new SMA(ParticlesCommandOptions.nbParticles, ParticlesCommandOptions.envWidth, ParticlesCommandOptions.envHeight, ParticlesCommandOptions.agentSize, ParticlesCommandOptions.toroidal, ParticlesCommandOptions.equity)
    model.init()
    primaryStage.setTitle("Particles")
    val scene = new Scene(canvas, ParticlesCommandOptions.envWidth, ParticlesCommandOptions.envHeight)
    ...
    if(ParticlesCommandOptions.nbTicks > 0)
      timeLineLoop.setCycleCount(ParticlesCommandOptions.nbTicks)
    else
